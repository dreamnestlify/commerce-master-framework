...
echo "  Done — Memory limit set to 512M"
...
# 3. Copy scripts into container, then run jewelry product setup
echo ""
echo "3/5 — Copying scripts to container & creating jewelry products..."
$COMPOSE cp scripts/jewelry-product-setup.php wordpress:/tmp/jewelry-product-setup.php
$COMPOSE exec wordpress bash -c "php -d memory_limit=512M /usr/local/bin/wp eval-file /tmp/jewelry-product-setup.php --allow-root"

# 4. Run site design setup
echo ""
echo "4/5 — Configuring homepage, menu & theme..."
$COMPOSE cp scripts/site-design.php wordpress:/tmp/site-design.php
$COMPOSE exec wordpress bash -c "php -d memory_limit=512M /usr/local/bin/wp eval-file /tmp/site-design.php --allow-root"
